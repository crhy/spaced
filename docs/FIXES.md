...
| 57 MB settings package on every APT fix | Artwork is ~90% of `spaced-mate-default-settings` (~53 MB wallpapers, ~6 MB themes/icons) yet rarely changes, so every desktop fix re-shipped ~57 MB into the GitHub Pages-hosted APT repository, which has a 1 GB site limit. The package is split into `spaced-wallpapers` (independently versioned), `spaced-themes` (themes, icons, GRUB artwork, icon-cache refresh), and a leaner `spaced-mate-default-settings` depending on both, with Breaks/Replaces takeovers for upgrades. | `packages/spaced-wallpapers/`, `packages/spaced-themes/`, `scripts/iso/build-local-packages.sh` |
| Same-version package rebuilt with new bytes (9.26.4) | `dpkg-deb` clamps only timestamps newer than `SOURCE_DATE_EPOCH`; older checkout times stayed in the archive, so `spaced-wallpapers 2026.09-1` rebuilt from a new worktree differed from the published bytes and `build-apt-repo.sh` refused it. `build()` now sets every staged timestamp to `SOURCE_DATE_EPOCH`, and the package was republished as `2026.09-2`. The Marco chroot build is also not reproducible across Ceres snapshots: before `make iso-build` or `make apt-repo`, copy the published `libmarco-private2`, `marco` and `marco-common` debs of the pinned version into `build/marco/artifacts/` instead of rebuilding them. | `scripts/iso/build-local-packages.sh`, `packages/spaced-wallpapers/DEBIAN/control` |
| Theme switch stuck after using Cairo-dock (#213, 9.26.2-2) | `spaced-switch-theme` serializes switches with `flock` on fd 7. A `flock` is released only when *every* copy of that descriptor closes, so any long-lived child started from the script (here `cairo-dock &`) must close it with `7>&- 8>&-`. Otherwise the child holds the lock for its lifetime and every later switch silently blocks before moving the panel. Apply the same rule to any new background process launched from a locked helper. | `overlays/usr/local/bin/spaced-switch-theme` |
| Upgrade offered again after it succeeded (#209, 9.26.2-2) | Primary cause: Spaced Update offers an OS release when the newest GitHub release tag is newer than `VERSION_ID`, but installs it from `crhy/spaced-apt`. The v9.26.2 tag and ISO were published without running `make apt-repo-publish`, so APT had nothing to install. **A release is not published until the APT repository carries its `spaced-meta` revision**; publish APT before or with the GitHub release. Secondary safeguard: `/etc/os-release` is written by `spaced-meta`'s postinst, which also runs as a `noawait` trigger on `/etc/os-release` and `/usr/lib/os-release`. `apt` can return success with triggers still pending, leaving `VERSION_ID` at the old release. `spaced-update-helper` now runs `dpkg --configure --pending` and `dpkg --triggers-only --pending` after the APT transaction, and the GUI rereads the installed version on every release check. Anything that compares against `VERSION_ID` must do the same. | `spaced-update-helper`, `spaced-update.py`, `packages/spaced-meta/DEBIAN/triggers` |
| Seeded Brave bookmarks ignored | The skel `Bookmarks` file carries a Chromium checksum that `scripts/check.sh` recomputes. Any edit to names, URLs or ids must recompute it, and the bookmark order is asserted exactly. Skel changes reach only newly created profiles. | `overlays/etc/skel/.var/app/com.brave.Browser/...`, `scripts/check.sh` |
| Fixes that live outside this repository | `components/spacedwelcome` and `components/spacedbazaar` are separate Git repositories (`crhy/spacedwelcome`, `crhy/spacedbazaar`). Their fixes need their own PR and release: Welcome reaches systems as a Debian dependency of `spaced-meta`, and Bazaar through the `spaced-github` Flatpak origin, not through Spaced Update/APT. A commit in this repository cannot fix them. | `components/`, `spaced-meta` control, external-artifact config |
| Themed icon overrides must use the exact icon name the application requests and live in a Spaced-Menu-On-* or Spaced-Icons-* theme, because hicolor is searched after Papirus. | Caja requests `network-workgroup` for the Network Servers desktop item, so the 9.26.2 gray hicolor `network-server` fallback never rendered. The gray artwork now ships as `network-workgroup.svg` (plus `network-server.svg`) in both Spaced-Menu-On-* themes. | `overlays/usr/share/icons/Spaced-Menu-On-*/scalable/places/`, `scripts/check.sh` |
