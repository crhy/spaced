...
| KVM input/display drift | Interactive launchers use one standard VGA device, GTK, USB keyboard/tablet, KVM acceleration, and SSH forwarding on port 2222. The unattended smoke path uses VNC plus a framebuffer capture and verifies the full MATE desktop. A second virtio GPU, QXL/SPICE, and legacy 6.26 launchers were removed. |
