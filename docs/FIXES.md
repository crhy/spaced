...
| Upgrade offered again after it succeeded (#209, 9.26.2-2) | `/etc/os-release` is written by `spaced-meta`'s postinst, which also runs as a `noawait` trigger on `/etc/os-release` and `/usr/lib/os-release`. `apt` can return success with triggers still pending, leaving `VERSION_ID` at the old release. `spaced-update-helper` now runs `dpkg --configure --pending` and `dpkg --triggers-only --pending` after the APT transaction, and the GUI rereads the installed version on every release check. Anything that compares against `VERSION_ID` must do the same. | `spaced-update-helper`, `spaced-update.py`, `packages/spaced-meta/DEBIAN/triggers` |
