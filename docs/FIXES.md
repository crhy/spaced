...
| Theme warning / question mark | All eleven themes contain GTK 2, GTK 3, Marco, component metadata, top-level metatheme metadata, wallpaper metadata, and a valid icon theme. |
...
| Compiz silently falling back | The profile moved to `.config/compiz-1`, includes the `ccp` plugin, and Compiz starts with the known-good `compiz ccp --replace` command. Marco remains an explicit fallback. |
